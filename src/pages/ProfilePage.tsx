  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
      <div>
        ...
        <p className="text-neutral-400 mt-1">Manage your identity on StrawberrySMP.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="space-y-6">
          <div className="bg-neutral-900 border border-neutral-800 rounded-3xl p-6 text-center">
            ...
            <p className="text-sm text-neutral-500 capitalize mb-6">{profile?.role}</p>
            
            <div className="flex flex-col gap-2">
              <div className="flex items-center justify-between text-xs p-3 bg-neutral-800/50 rounded-xl">
                <span className="text-neutral-500 font-bold uppercase tracking-wider">Rank</span>
                <span className="text-strawberry-500 font-bold">{profile?.role === 'admin' ? 'STAFF' : 'MEMBER'}</span>
              </div>
              <div className="flex items-center justify-between text-xs p-3 bg-neutral-800/50 rounded-xl">
                <span className="text-neutral-500 font-bold uppercase tracking-wider">Joined</span>
                <span className="text-white">{profile?.created_at ? new Date(profile.created_at).toLocaleDateString() : 'N/A'}</span>
              </div>
            </div>
          </div>
          ...
          <button 
            onClick={() => signOut()}
            className="w-full flex items-center justify-center gap-2 p-4 bg-neutral-900 border border-neutral-800 rounded-2xl text-red-500 font-bold hover:bg-red-500/5 transition-all"
          >
            <LogOut size={20} />
            Sign Out Session
          </button>
        ...
        <div className="lg:col-span-2">
          <form onSubmit={handleUpdate} className="bg-neutral-900 border border-neutral-800 rounded-3xl p-8 space-y-6">
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-neutral-400">Bio / About You</label>
              <textarea 
                value={formData.bio}
            ...
            <div className="pt-4">
              <button 
                disabled={loading}
                type="submit"
                className="w-full py-4 bg-strawberry-600 rounded-2xl font-bold hover:bg-strawberry-700 transition-all flex items-center justify-center gap-2 shadow-lg shadow-strawberry-600/20"
              >
