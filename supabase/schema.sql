...
CREATE POLICY "Admins can manage rules" ON rules FOR ALL 
  USING (EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'admin'));

CREATE POLICY "Reminders are viewable by everyone" ON reminders FOR SELECT USING (true);
CREATE POLICY "Admins can manage reminders" ON reminders FOR ALL 
  USING (EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'admin'));

CREATE POLICY "Versions are viewable by everyone" ON minecraft_versions FOR SELECT USING (true);
CREATE POLICY "Admins can manage versions" ON minecraft_versions FOR ALL 
  USING (EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'admin'));
